...
(function(root, factory) {
  ...
  //Workaround requestAnimationFrame & cancelAnimationFrame

  const FPS = 1000 / 60;
  var runningInstances = [];

  function requestAnimationFrame(cb) {
    var p = runningInstances.length;
    runningInstances[p] = true;
    setTimeout(function() {
      console.log("hier");
      if (runningInstances[p]) {
        cb();
      }
    }, FPS);
    return p + 1;
  ...
  function cancelAnimationFrame(p) {
    if (p - 1 >= 0) runningInstances[p - 1] = true;
  }
